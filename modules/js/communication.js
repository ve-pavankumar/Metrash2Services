...
//	var password = "iyadaw";
    ...
    var inputParams={serviceID:"residencyComposite",qid:qid};
 		...
 		if(null != output && null != output["opstatus"] && 0 == output["opstatus"])
	  	{
			...
			{
				var basicConf = {message:"Service did not return any records", alertType:constants.ALERT_TYPE_INFO, alertTitle:"", yesLabel:"OK", noLabel:"", alertHandler:null};
				var pspConf = {};
...
function detailsIconClicked(){
    ...
    if(rejectionCode!=null){
		...
		{lblPersResKey:kony.i18n.getLocalizedString("rr.dob"),lblPersResValue:selectedSegmentData.dob},
		{lblPersResKey:kony.i18n.getLocalizedString("m.ps.epf.detail.age"),lblPersResValue:""},
		{lblPersResKey:kony.i18n.getLocalizedString("m.residencyDate"),lblPersResValue:""},
		{lblPersResKey:kony.i18n.getLocalizedString("m.passport"),lblPersResValue:selectedSegmentData.passportNo},
		{lblPersResKey:kony.i18n.getLocalizedString("m.ps.expiryDate"),lblPersResValue:selectedSegmentData.passportExpDate},
		{lblPersResKey:kony.i18n.getLocalizedString("m.residency.fees"),lblPersResValue:""},
		{lblPersResKey:kony.i18n.getLocalizedString("m.residency.delayFees"),lblPersResValue:selectedSegmentData.delayFees},
		{lblPersResKey:"Entry Date",lblPersResValue:""}
		];
...
recidencyrenwalObject.totalAmount=kony.os.toNumber(recidencyrenwalObject.totalAmount)||"0";
	...
	if(frmPersonalResidencyInput.btnQPost.skin == "btnCheckbox"){
		...
		if(frmPersonalResidencyInput.btnImmigration.skin == "btnCheckBoxClicked")
...
recidencyrenwalObject.totalAmount=kony.os.toNumber(recidencyrenwalObject.totalAmount)||"0";
	...
	if(frmPersonalResidencyInput.btnImmigration.skin == "btnCheckbox"){
		...
		if(frmPersonalResidencyInput.btnQPost.skin == "btnCheckBoxClicked")
			recidencyrenwalObject.qpostOrImmigrationAmount = kony.os.toNumber(recidencyrenwalObject.qpostOrImmigrationAmount) - kony.os.toNumber(residencyPersonalRenewalDeliveryFeesObject.QPostFees);
		frmPersonalResidencyInput.btnQPost.skin = "btnCheckbox";
		recidencyrenwalObject.qpostOrImmigrationAmount  =kony.os.toNumber(recidencyrenwalObject.qpostOrImmigrationAmount) + kony.os.toNumber(residencyPersonalRenewalDeliveryFeesObject.immigrationFees);
...
function assignCheckBoxClickForSponseredItems(rejectedCode)
{
	...
	if(rejectedCode!="")
					{
						vbxPersResCheckboxEvent={visible:"false"};
					}
	else
	vbxPersResCheckboxEvent={visible:"true"};
...
function assignYearRejectedQID(rejectedCode,bntText)
{
	...
	if(rejectedCode!="")
					{
						yearButtonItem={"text":bntText,visible:"false"};
					}
	else
	yearButtonItem={"text":bntText,visible:"true"};
...
function personalResidencyInputScreenValidations(){
	...
	if(numberOfSelectedRecords == 0){
		var basicConf = {message:"Please Select minimum one record", alertType:constants.ALERT_TYPE_INFO, alertTitle:"Residency Renewal", yesLabel:"OK",alertHandler:null};
		var pspConf = {};
		var infoAlert = kony.ui.Alert(basicConf,pspConf);
		return;
	}
	...
	if(residencyPersonalRenewalConfigurationObject.showPersRenDeliveryMethodSection != "0"){
		if(frmPersonalResidencyInput.btnQPost.skin == "btnCheckbox" && frmPersonalResidencyInput.btnImmigration.skin == "btnCheckbox"){
			var basicConf = {message:"Please Select Delivery Option", alertType:constants.ALERT_TYPE_INFO, alertTitle:"Residency Renewal", yesLabel:"OK",alertHandler:null};
			var pspConf = {};
			var infoAlert = kony.ui.Alert(basicConf,pspConf);
			return;
		}else{
